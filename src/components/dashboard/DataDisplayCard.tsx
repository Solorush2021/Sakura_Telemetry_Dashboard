...
export function DataDisplayCard({ title, value, unit, icon, className }: DataDisplayCardProps) {
  return (
    ...
    >
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-4 px-16">
        <CardTitle className="text-base font-medium text-muted-foreground uppercase tracking-wider text-center flex-grow">{title}</CardTitle>
        <div className="text-primary animate-pulse-scale-glow">{icon}</div>
      </CardHeader>
      <CardContent className="flex-grow flex flex-col justify-center p-6 pt-0 px-16 text-center">
