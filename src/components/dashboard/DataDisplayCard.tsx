...
export function DataDisplayCard({ title, value, unit, icon, className }: DataDisplayCardProps) {
  return (
    <Card 
      ...
      style={{ clipPath: 'polygon(12% 0%, 100% 0%, 88% 100%, 0% 100%)' }} // Slightly less aggressive clip
    >
      {/* Removed top accent line, neon-outline provides border effect */}
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 pt-6 px-12"> {/* Increased padding */}
      ...
      <CardContent className="flex-grow flex flex-col justify-center p-4 pt-0 px-12"> {/* Increased padding */}
        <div className="text-5xl font-bold text-foreground text-center"> {/* Added text-center */}
          {value}
